...
df <- transform_df(simulate_data_real(n = 100)$dataframe, score_max = 11)
mean(df$T)
...
fit_ <- MIC(df, user_formula = "cont + factor(sex) + factor(race)", method = "po")
fit$meanT
fit_$meanT
...
p_result <- run_simulations(setup = 0, m = 100, sim_model = c("simulate_data_real"), model = c("po"))
print(p_result)
