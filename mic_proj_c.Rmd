...
simulate_data_dep <- function(n, max_score, seed = NULL, setup = 0) {
  ...
  if(setup == 0){
    ...
    T <- numeric(n)
    for(i in 1:n) {
      T_probs <- exp(eta * X[i] + mat[ , race[i] + 1])
      T_probs <- T_probs / sum(T_probs)  
      
      D_probs <- exp(eta_ * X[i] + race[i])
...
```{r}
max_score <- 5
data_oracle <- simulate_data_dep(1000, max_score, setup = 1)
...
result <- MIC(df, method = "ph", user_formula = "X + factor(race)", cov = data.frame(race = race_, X = 1))
result$meanT
