...
simulate_data_po <- function(n, seed = NULL, setup = 0, cov = NULL) {
  ...
  # Specify the baseline odds for the event time T (times 1 to 10)
  baseline_odds_T <- c(0.01, 0.03, 0.1, 0.2, 0.3, 0.35, 0.4, 0.5, 0.6, 0.04)
  
  # Set coefficients for the linear predictors for T and Delta
  ...
  # Override generated covariate values if fixed covariates are provided
  if(!is.null(cov)){
  ...
  # Compute the linear predictor for T and generate event times using the proportional odds model
  ...
  sigma <- 2  # Spread parameter; lower values concentrate probability more strongly at 3.
  # Baseline log-probabilities: highest at 3 and falling off symmetrically.
  eta <- -((possible_delta - 3)^2) / (2 * sigma^2)
  ...
  Delta <- sapply(1:n, function(i) {
    # For individual i, incorporate the covariate effect linearly (shifting the baseline)
    unnormalized <- exp(eta + possible_delta * lp_delta[i])
...
po_sim <- function(n, m, setup, cov = NULL){ 
  # Use a large sample to estimate the true distribution of T
  ...
  for(i in 1:m){
    ...
    data_observed <- subset(data_oracle, select = -T) # Data frame without T
  ...
  class(result) <- "po_sim"
  return(result) # May need to skip nulls (caused by Max iteration reached)
}
